 ...
 * 3. 支持添加/删除明细行
 * 4. 提交按钮（调用 createOutboundOrder API）
 */
  ...
  <div>
    ...
    <el-form label-width="100px" style="max-width: 900px">
      <el-form-item label="客户名称" required>
        <el-input v-model="customerName" placeholder="请输入客户名称" maxlength="200" />
      </el-form-item>
    ...
    <el-table v-if="items.length > 0" :data="items" border style="width: 100%; margin-bottom: 16px">
      ...
      <el-table-column label="商品" min-width="200">
        <template #default="{ row }">
          <el-select v-model="row.productId" filterable placeholder="请选择商品" style="width: 100%">
            <el-option
              v-for="p in products"
              :key="p.id"
              :label="`${p.name} (${p.sku})`"
      ...
      <el-table-column label="库位" width="160">
        <template #default="{ row }">
          ...
          >
            <el-option v-for="loc in row.locations" :key="loc.code" :label="loc.code" :value="loc.code" />
          </el-select>
        </template>
      </el-table-column>
